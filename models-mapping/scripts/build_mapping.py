...
* ``data/all_models.json`` -- flat vendor/model catalog (models.dev/models.json);
* ``data/opencode-go-models.json`` -- opencode-go provider enriched with Go
  limits/prices (go.mdx merged inline via watch_go);
* ``data/arena.json`` -- the Arena leaderboard;
* ``config/request-models.json`` -- the configured Claude/GPT request IDs.

The source model set is the exact intersection of bare all_models IDs and
opencode-go Go-enriched models.  Joining those two sources never applies name
fallbacks.  Arena joins retain the historical fallback chain from
:mod:`name_matching` and record warnings for low-confidence matches.  Missing
source details are reported but do not remove a model from the generated
workspace; incomplete candidates simply do not participate in target selection.
"""
...
from csv_io import MAPPING_COLUMNS, write_mapping  # noqa: E402
from name_matching import find_best_match, normalize_arena_name  # noqa: E402


SCHEMA_VERSION = 1
DEFAULT_MODELS_PATH = Path("data/all_models.json")
DEFAULT_GO_PATH = Path("data/opencode-go-models.json")
DEFAULT_GOAT_PATH = Path("data/goat-models.json")
...
DEFAULT_ENRICHED_PATH = Path("data/enriched.json")
...
def _is_models_flat_vendor_map(payload: Any) -> bool:
    """Return True when payload is the flat vendor/model map (all_models.json)."""

    if not isinstance(payload, Mapping) or not payload:
        return False
    if "providers" in payload or "schema_version" in payload or "models" in payload:
        return False
    return all("/" in str(key) for key in payload.keys())


def _is_opencode_go_envelope(payload: Any) -> bool:
    """Return True when payload is the opencode-go envelope with Go fields merged inline."""

    if not isinstance(payload, Mapping):
        return False
    provider = payload.get("opencode-go")
    return isinstance(provider, Mapping) and isinstance(provider.get("models"), Mapping)


def _is_goat_envelope(payload: Any) -> bool:
    """Return True when payload is the api.json-shaped goat snapshot."""

    if not isinstance(payload, Mapping):
        return False
    provider = payload.get("commandcode-goat")
    return isinstance(provider, Mapping) and isinstance(provider.get("models"), Mapping)
...
def _validate_envelope(
    ...
    """Require the agreed JSON envelope; skip for the new all_models / opencode-go shapes."""
    ...
    if not isinstance(payload, dict):
        ...
        return
    if source_name == "models.json" and _is_models_flat_vendor_map(payload):
        return
    if source_name == "go.json" and _is_opencode_go_envelope(payload):
        return
...
def load_models_data(payload: Any, report: Optional[dict] = None) -> Dict[str, dict]:
    """Load model IDs from the flat vendor/model map (data/all_models.json). Only this shape is accepted."""
    ...
    if not isinstance(payload, Mapping):
        target_report["errors"].append(
            _report_item("invalid_models_source", "models.json must be an object")
        )
        return models

    if not _is_models_flat_vendor_map(payload):
        target_report["errors"].append(
            _report_item("invalid_models_source", "models.json must be the flat vendor/model map (all_models.json)")
        )
        return models
    ...
    for key, value in payload.items():
        ...
        if not bare_id:
            bare_id = str(value.get("id") or key).strip()
        if bare_id and bare_id not in models:
            models[bare_id] = {**dict(value), "provider": str(key).split("/", 1)[0]}
        elif bare_id:
            target_report["warnings"].append(
                _report_item(
                    "duplicate_model_id",
                    f"model {bare_id!r} appears more than once in models.json",
                    model_id=bare_id,
                )
            )
    if not models:
...
def load_go_data(payload: Any, report: Optional[dict] = None) -> Dict[str, dict]:
    """Load Go extension records from the enriched opencode-go envelope. Only this shape is accepted."""
    ...
    if not isinstance(payload, Mapping):
        ...
        return {}
    if not _is_opencode_go_envelope(payload):
        target_report["errors"].append(
            _report_item("invalid_go_source", "go.json must be the opencode-go envelope (opencode-go-models.json)")
        )
        return {}
    payload = payload["opencode-go"].get("models", payload)
    source_models = payload.get("models", payload) if isinstance(payload, Mapping) else payload
    models: Dict[str, dict] = {}
...
def load_goat_data(payload: Any, report: Optional[dict] = None) -> Dict[str, dict]:
    """Load candidate records from data/goat-models.json (commandcode-goat provider)."""

    target_report = report or {"errors": [], "warnings": []}
    if not isinstance(payload, Mapping):
        target_report["errors"].append(
            _report_item("invalid_goat_source", "goat-models.json must be an object")
        )
        return {}
    provider = payload.get("commandcode-goat")
    if not (isinstance(provider, Mapping) and isinstance(provider.get("models"), Mapping)):
        target_report["errors"].append(
            _report_item(
                "invalid_goat_source",
                "goat-models.json must contain a commandcode-goat provider with models",
            )
        )
        return {}
    models: Dict[str, dict] = {}
    for key, value in provider["models"].items():
        if not isinstance(value, Mapping):
            target_report["errors"].append(
                _report_item("invalid_goat_model", f"goat model {key!r} is not an object")
            )
            continue
        model_id = str(key).strip()
        if not model_id:
    ...
    if not models:
        target_report["errors"].append(
            _report_item("empty_goat_source", "goat-models.json contains no models")
        )
    return models
...
def _fix_free_records(records: Dict[str, dict]) -> Dict[str, dict]:
    ...
    fixed, _ = _fix_free_records_with_provenance(records)
    return fixed


def _fix_free_records_with_provenance(
    records: Dict[str, dict],
) -> Tuple[Dict[str, dict], Dict[str, dict]]:
    """Apply the free-default rule; return patched records and derived fields per model.
    ...
    for model_id, value in records.items():
        ...
        if _missing(value.get("rp5h")):
            ...
            filled["rp5h"] = value["rp5h"]
        if _missing(value.get("usage_quota")):
            value["usage_quota"] = 60
            filled["usage_quota"] = 60
        for field in (
            "price_input",
            "price_output",
            "max_price_output",
...
def build_mapping_data(
    ...
    goat_payload: Any | None = None,
) -> Tuple[List[dict], dict]:
    """Build CSV rows and a structured report from the in-memory sources."""
    ...
    goat_models = load_goat_data(goat_payload, report) if goat_payload is not None else {}
    ...
    for model_id, fields in decisions["supplements"].items():
        target = go_models if model_id in go_models else goat_models
        target.setdefault(model_id, {}).update(fields)
    go_models, go_derived = _fix_free_records_with_provenance(
        {key: dict(value) for key, value in go_models.items()}
    )
    goat_models, goat_derived = _fix_free_records_with_provenance(
        {key: dict(value) for key, value in goat_models.items()}
    )
    enrichment: Dict[str, dict] = {}
    for source_name, derived, source_records in (
        ("opencode-go", go_derived, go_models),
        ("commandcode-goat", goat_derived, goat_models),
    ):
        for model_id, fields in derived.items():
            enrichment[model_id] = {
                "source": source_name,
                "derived": fields,
                **{field: source_records[model_id].get(field) for field in fields},
            }

    request_ids = {str(item["model_id"]) for item in requests}
    candidate_universe = (set(go_models) | set(goat_models)) - request_ids
    manual_excluded = set(decisions["excluded"])
    for model_id in sorted(candidate_universe & manual_excluded):
    ...
    candidate_ids = candidate_universe - manual_excluded
    max_source_rp5h = max(
    ...
    for model_id in sorted(candidate_ids):
        source_record = (
            go_models.get(model_id)
            or goat_models.get(model_id)
            or {}
        )
        go_record = dict(source_record)
        ...
        if arena_entry is None or match_type == "no_match":
            ineligible_reasons.append("missing_arena")
            report["ineligible"].append(
                {
                    "model_id": model_id,
                    "reason": "missing_arena",
                }
            )
        else:
        ...
        for field in ("rp5h",):
            if _number(go_record.get(field)) is None:
                ...
                ineligible_reasons.append(f"missing_{field}")
                report["ineligible"].append(
                    {
                        "model_id": model_id,
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Build models.csv from the source snapshots")
    parser.add_argument("--models", type=Path, default=DEFAULT_MODELS_PATH)
    parser.add_argument("--go", type=Path, default=DEFAULT_GO_PATH)
    parser.add_argument(
        "--goat",
        type=Path,
        default=DEFAULT_GOAT_PATH,
        help="goat snapshot added to the candidate universe (default: data/goat-models.json)",
    )
    ...
    parser.add_argument(
        ...
        help="path for the derived-only enrichment record (free-default fills); pass empty to skip",
    )
    ...
    try:
        ...
        goat_payload = _read_json(args.goat, "goat-models.json") if args.goat else None
        ...
        if args.enriched_output:
            _write_json_atomic(args.enriched_output, enrichment)
    ...
    if args.enriched_output:
        print(f"build-mapping: wrote enrichment for {len(enrichment)} models to {args.enriched_output}")
