...
- 快照推送（ADR 0010）在 vol-server 上持有专用服务账号凭据（`AXONHUB_EMAIL` + root-only 密码文件，见 `axonhub-admin/deploy-vol-server-push.md`），每次运行换一次性 JWT；该凭据不出服务器，CI 与交互会话都不使用它。
...
- 唯一的无人值守写入是快照推送（ADR 0010）：vol-server timer 经 `apply_channel_models.py` 只写 `external_channel_lists` 渠道的 `supportedModels` 与 `autoSyncSupportedModels=false`，幂等 + 回读验证，失败保留现状退出；交互会话不得手改这些渠道的清单（下轮 timer 会还原），改快照事实源。
- 写入只发生在交互式 agent 会话或上述快照推送 timer；CI（GitHub Actions）永不写 AxonHub，也不持有其凭据。
