...
系统分三层：采集（watch-pipeline：CI 只抓公开数据进仓库）→ 计算（model-registry：离线去重、补卡、Claude 映射建议与 channel/model 双 plan）→ 确认后写入（axonhub-admin）。每层只做自己的事；归属细节见 `README.md`。
...
- 改变拥有或写入边界 → [`docs/adr/`](docs/adr)（当前 0005–0017，0012 为现行架构，0015/0016/0017 为现行匹配、路由与产物口径），再改本文
...
规划默认写出 `models.csv` 与 `data/{channel-plan,model-plan}.json`（生成物，整体重算，不手工编辑）；单模型卡终态用 `.agents/skills/model-registry/scripts/assemble_card.py --id <modelID>` 在写入前组装。

采集脚本位于 `.agents/skills/watch-pipeline/scripts/`，由 `watch-pipeline` skill 维护并经 watch-pipeline.yml（每 3 天）执行；本地运行 watch_* 仅作调试，产物仍归对应渠道。go 与 goat 两个采集 job 都写 `data/models_extra.json` 的各自节，必须串行（CI 已按 go → goat 排序）。脚本失败会在 `.agents/skills/watch-pipeline/reference/<channel>/last-error.json` 留痕，修复流程见 `.agents/skills/watch-pipeline/SKILL.md`。skill 校验见各自 `SKILL.md`。

## 约束

- Python 3.12+，只用标准库；公开函数带类型注解。
- `data/models_extra.json` 渠道节内记录的契约字段由对应采集脚本独占写入，按字段级合并（新 id 插入、消失 id 连记录删除，契约外字段保留）；记录上契约外的字段（如 `exclude`）与顶层 `aliases` 为人工维护；例外：`ant`、`sensenova` 为人工维护静态节（ADR 0013），任何 watcher 不得触碰。模型卡只来自 `data/all_models.json`，渠道 `cost` 逐字段优先；仅 Arena 允许使用文档化的 fallback 链。
...
- `data/arena.json` 的 `manual: true` 记录与 `models_extra.json` 的 `aliases`、记录级 `exclude` 是手工维护数据，采集/重生成必须保留（脚本已保证，不得绕过脚本直写文件）。
