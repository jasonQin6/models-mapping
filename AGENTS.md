...
- `data/models_extra.json`（仅 `channels` 节与顶层 `aliases`）渠道节内记录的契约字段由对应采集脚本独占写入，按字段级合并（新 id 插入、消失 id 连记录删除，契约外字段保留）；免费语义即 `cost` 零价：采集器把渠道免费措辞转写为零价（静态节人工声明），计算层只读价格判定，无旗标或 id 后缀启发式；顶层 `aliases` 人工维护；例外：`ant`、`sensenova` 为人工维护静态节（ADR 0013），任何 watcher 不得触碰。`data/blocklist.json`（channel → `[{id, reason}]`，唯一排除源）为 axonhub-admin 独占：`speed:`/`lowscore:` 两类由 channel-sync 脚本（`channel_sync.py`）物化重建，`manual`/`tier`/`retired` 等人工类维护者手写；采集层不读写该文件。模型卡只来自 `data/all_models.json`，渠道 `cost` 逐字段优先；仅 Arena 允许使用文档化的 fallback 链。
...
- `data/arena.json` 的 `manual: true` 记录、`models_extra.json` 的 `aliases`、`data/blocklist.json` 的人工类条目是手工维护数据，采集/重生成必须保留（脚本已保证，不得绕过脚本直写文件）。
