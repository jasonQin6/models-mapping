...
This skill is the maintenance manual for the collection layer. It owns `watch-pipeline/scripts/` (the `watch_*.py` scrapers plus `models_extra.py`, the shared models-extra store, and `error_state.py`), their tests, and the per-channel `reference/` directories. It does NOT own `model-registry/` (planning) and never writes AxonHub. Collection stores raw facts only: name normalization, matching, and derived-value backfill (free quotas, prices) live in the planning layer.
