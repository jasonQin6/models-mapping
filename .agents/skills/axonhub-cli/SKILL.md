---
name: axonhub-cli
description: Ad-hoc AxonHub operations over its GraphQL API via graphql-cli — queries, mutations, schema discovery, and endpoint management for channels, API keys, users, projects, and system settings. Use for one-off CLI work and generic tool mechanics; this repository's governed writes (channel-plan, model-plan, mapping table) belong to the axonhub-admin skill.
---
...
- Governed writes — this repository's channel-plan, model-plan, and mapping-table writes — run only through `axonhub-admin`'s interactive program; this skill is for one-off operations.
